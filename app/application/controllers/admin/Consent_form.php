...
class Consent_form extends AdminController
{
    ...
    /* =========================================================
       0) OLD PATIENT ID -> CURRENT (MAIN AUTOINCREMENT) RESOLVER
       ---------------------------------------------------------
       Purpose: some places (old print-outs, old QR / links, old
       imported records) still pass the PATIENT's OLD id instead
       of the current tblclients.userid (the "main" autoincrement
       id used everywhere - appointments, xray files in tblfiles,
       profile image folder, medical history, etc).

       This function always returns the CURRENT userid to use:
         1. If $id already exists in tblclients.userid -> return as is.
         2. Else try to match it as an OLD id:
              a. tblcontacts.old_userid = $id   (old client/patient id)
              b. tblcontacts.id          = $id   (old was actually a contact id)
            and resolve to that contact's CURRENT userid.
         3. Else, only as a last resort and only when a mobile
            number is supplied, try a phone match against
            tblcontacts. If more than one contact shares that
            number, we refuse to guess (returns 0) so we never
            silently attach x-ray/profile data to the wrong patient.

       Returns (int) current userid, or 0 if nothing could be
       safely resolved.
    ==========================================================*/
    private function resolve_current_patient_id($id, $phone = '')
    {
        ...
        // 1) Already a valid CURRENT patient id? Nothing to do.
        $exists = $this->db
            ->select('userid')
        ...
        // 2a) Try as an OLD patient id preserved on tblcontacts.old_userid
        $byOldUserid = $this->db
            ->select('userid')
        ...
        // 2b) Try treating the given id as an old tblcontacts.id
        $byContactId = $this->db
            ->select('userid')
            ->from(db_prefix() . 'contacts')
            ->where('id', $id)
        ...
        // 3) Last resort: exact phone match, but only if it is unambiguous.
        ...
        if ($phone !== '') {
            ...
            // more than one patient shares this phone number ->
    ...
    public function all_forms_json()
    {
        ...
        $patient_id = $this->resolve_current_patient_id($patient_id);
        ...
        if (!empty($form_ids)) {
            $submissions = $this->db
                ->select('id, consent_pdf_id, appointment_id, appointment_type_id, patient_name, doctor_id, doctor_name, updated_at, created_at, form_data_json')
                ->from(db_prefix() . 'consent_form_submissions')
                ->where('patient_id', $patient_id)
                ->where_in('consent_pdf_id', $form_ids)
                ->order_by('id', 'DESC')
                ->get()
                ->result_array();
        }
        ...
        foreach ($submissions as $submission) {
            $form_id = (int) ($submission['consent_pdf_id'] ?? 0);
            if ($form_id <= 0) {
                continue;
            }
    ...
    public function patient_history_pdf($patient_id = 0)
    {
     ...
     $prescriptions = [];
        ...
        $submissions = $this->db
            ->select(
                db_prefix() . 'consent_form_submissions.*, ' .
                'tblconsent_form_master.pdf_name, ' .
            ...
            ->from(db_prefix() . 'consent_form_submissions')
            ->join('tblconsent_form_master', 'tblconsent_form_master.pdf_id = ' . db_prefix() . 'consent_form_submissions.consent_pdf_id', 'left')
            ->join(db_prefix() . 'appointly_appointments', db_prefix() . 'appointly_appointments.id = ' . db_prefix() . 'consent_form_submissions.appointment_id', 'left')
            ->join(db_prefix() . 'staff', db_prefix() . 'staff.staffid = ' . db_prefix() . 'consent_form_submissions.doctor_id', 'left')
            ->where(db_prefix() . 'consent_form_submissions.patient_id', $patient_id)
            ->order_by(db_prefix() . 'consent_form_submissions.updated_at', 'DESC')
            ->order_by(db_prefix() . 'consent_form_submissions.id', 'DESC')
            ->get()
            ->result_array();
...
$rx_start_date = (string) ($primary_contact->rx_str_date ?? '');
        ...
        $html .= '<h3>Consent Forms Filled History</h3>';
        $html .= '<table><thead><tr>'
            . '<th style="width:35px;">#</th>'
            . '<th style="width:85px;">Date</th>'
            . '<th style="width:70px;">Appt ID</th>'
            . '<th style="width:170px;">Consent Form</th>'
            . '<th style="width:120px;">Doctor</th>'
            . '</tr></thead><tbody>';

        if (empty($submissions)) {
            $html .= '<tr><td colspan="6" style="text-align:center;">No consent form history found.</td></tr>';
        } else {
    ...
    public function form($pdf_id)
    {
        ...
        // Only English templates exist today, so default to 'en'
        // (not 'gu') whenever lang is missing/invalid. If Gujarati
        // templates are added later, requesting ?lang=gu still works.
        $lang = ($lang === 'gu') ? 'gu' : 'en';

        // 1️⃣ Get template
        $pdf = $this->db->where('pdf_id',$pdf_id)
                        ->get('tblconsent_form_master')
                        ->row_array();
        ...
        if ($fileName === '') {
            show_error('Template file missing');
        }

        // Forms live in a single flat folder — no english/gujarati
        // subfolders. Update this path if your real folder differs.
        $path = FCPATH . 'uploads/consern_form/' . basename($fileName);
        if (!file_exists($path)) {
        ...
        $this->db->where('consent_pdf_id',$pdf_id);
        $this->db->where('patient_id',$patient_id);
        $this->db->where('appointment_id',$appointment_id);
        $this->db->where('lang',$lang);

        $row = $this->db->order_by('id','DESC')
                        ->get(db_prefix().'consent_form_submissions')
                        ->row_array();
        ...
        // Same merge pattern print_pdf() already uses, so the
        // interactive View gets the same doctor/patient name +
        // signature auto-fill (by name/id match, and by scanning
        // the template for doctor/patient/signature context —
        // see apply_party_defaults_for_pdf()) that the PDF export
        // already had.
        if (!isset($saved['patient_name']) && $patient_name !== '') {
            $saved['patient_name'] = $patient_name;
        }
        if (!isset($saved['doctor_name']) && $doctor_name !== '') {
            $saved['doctor_name'] = $doctor_name;
        }
        if ((!isset($saved['doctor_signature_image']) || trim((string)$saved['doctor_signature_image']) === '') && $doctor_signature_image !== '') {
            $saved['doctor_signature_image'] = $doctor_signature_image;
        }
        if (!isset($saved['today_date'])) {
            $saved['today_date'] = date('d/m/Y');
        }

        // Fill the HTML server-side, same as print_pdf() — but
        // with forPdf=false so signature images resolve to real
        // web URLs the browser can load, not file:// paths.
        $html = $this->apply_saved_to_html_for_pdf($html, $saved, false);
    ...
    public function save_submission()
    {
        ...
        $pdf_id        = (int)$payload['consent_pdf_id'];
       ...
       if ($doctor_name === '' && $doctor_id > 0 && function_exists('get_staff_full_name')) {
        ...
        $table = db_prefix().'consent_form_submissions';
    ...
    private function _inject_common_script($ctx,$saved)
    {
        ...
        return <<<HTML
<script>
window.__CONSENT_CTX={$ctxJson};
window.__CONSENT_SAVED={$savedJson};
...
(function(){
  ...
  document.addEventListener('DOMContentLoaded',function(){
    var data=window.__CONSENT_SAVED||{};
  ...
  document.addEventListener('click',async function(e){
    ...
    var res=await fetch(CTX.admin_base+'consent_form/save_submission',{method:'POST',body:fd});
...
private function inject_global($html, $ctx, $saved)
{
    ...
    // Saving is handled by the parent page's Save button
    // (saveConsentForm() in patient_profile.php), which reads this
    // iframe's fields directly on click — so no external global JS
    // file is required here.
    //
    // Pre-filling the form (saved answers, doctor/patient name,
    // signatures) now happens server-side, above, via
    // apply_saved_to_html_for_pdf($html, $saved, false) — the same
    // proven logic print_pdf() uses — so by the time this HTML
    // reaches the browser it's already filled in. This script only
    // needs to expose CTX/SAVED for the Save button to read.
    $script = "
<script>
window.__CONSENT_CTX = {$ctxJson};
window.__CONSENT_SAVED = {$savedJson};
...

{
    ...
    if (!$req) {
        $req = [
            'consent_pdf_id'         => (int)$this->input->get('consent_pdf_id'),
            'appointment_id'      => (int)$this->input->get('appointment_id'),
            'appointment_type_id' => (int)$this->input->get('appointment_type_id'),
            'patient_id'          => (int)$this->input->get('patient_id'),
            'doctor_id'           => (int)$this->input->get('doctor_id'),
    ...
    $consent_pdf_id    = (int)($req['consent_pdf_id'] ?? 0);
    ...
    $respondTemplateMissing = function($msg) use ($isJson) {
        ...
        redirect($ref ?: admin_url('consent_form'));
        exit;
    };

    if ($consent_pdf_id <= 0 || $appointment_id <= 0) {
        return $this->output
            ->set_status_header(422)
            ->set_content_type('application/json')
            ->set_output(json_encode(['status' => false, 'message' => 'Missing consent_pdf_id / appointment_id']));
    }

    // 1) Load Consent Form master
    $master = $this->db->where('pdf_id', $consent_pdf_id)->get('tblconsent_form_master')->row();
    if (!$master) {
        return $this->output->set_status_header(404)->set_output('Consent form master record not found');
    }
    ...
    // 3) Load HTML from the flat consent form folder (no language subfolders)
    $path = FCPATH . 'uploads/consern_form/' . basename($file);
    ...
    if (empty($saved)) {
        $this->db->where('consent_pdf_id', $consent_pdf_id);
        $this->db->where('appointment_id', $appointment_id);
        $this->db->where('lang', $lang);
        $this->db->order_by('id', 'DESC');
        $row = $this->db->get(db_prefix() . 'consent_form_submissions')->row();
   ...
   // $html = $this->append_universal_signature_block($html);
    ...
    // 9) Choose engine
    $title    = trim((string)($master->pdf_name ?? 'Consent Form'));
...
{
    if (stripos($html, 'id="consent-form-universal-signatures"') !== false) {
        return $html;
    }

    $block = '<div id="consent-form-universal-signatures" style="margin-top:24px;">'
...
private function resolve_doctor_profile_image(int $doctor_id): string
{
   ...
   private function apply_saved_to_html_for_pdf($html, array $saved, bool $forPdf = true)
{
    ...
    $saved = $this->apply_party_defaults_for_pdf($xpath, $saved, $forPdf);

        foreach ($saved as $key => $val) {
        $key = (string)$key;

        $isImageValue = $this->is_signature_image_value((string)$val);
        $normalizedImageValue = $isImageValue
            ? ($forPdf ? $this->normalize_signature_src_for_pdf((string)$val) : $this->normalize_signature_src_for_view((string)$val))
            : '';
...
private function apply_party_defaults_for_pdf(DOMXPath $xpath, array $saved, bool $forPdf = true): array
{
    ...
    $this->apply_signature_images_for_pdf($xpath, $saved, $doctorSignImage, $patientSignImage, $forPdf);

    foreach ($xpath->query('//input|//textarea') as $node) {
        // Name/signature auto-fill only makes sense for text-like
        // fields. Checkboxes/radios can have doctor/patient/signature
        // words in their *label* purely descriptively — e.g. a consent
        // checklist item mentioning "...Medical Directors, and
        // Specialized Surgeons..." — which would otherwise get a name
        // string stashed as their saved value (harmless for rendering,
        // since only the checked attribute matters, but confusing in
        // the stored form_data_json).
        $tag = strtoupper($node->tagName ?? '');
        if ($tag === 'INPUT') {
            $inputType = strtolower(trim((string)$node->getAttribute('type')));
        ...
        if ($this->is_doctor_related_context($context) && $doctorName !== '') {
            $signText = ($doctorSign !== '' && !$this->is_signature_image_value($doctorSign)) ? $doctorSign : '';
            $fillValue = $this->is_signature_related_context($context)
                ? ($signText !== '' ? $signText : $doctorName)
                : $doctorName;
        } elseif ($this->is_patient_related_context($context) && $patientName !== '') {
            $signText = ($patientSign !== '' && !$this->is_signature_image_value($patientSign)) ? $patientSign : '';
            $fillValue = $this->is_signature_related_context($context)
                ? ($signText !== '' ? $signText : $patientName)
                : $patientName;
        }
...
private function apply_signature_images_for_pdf(DOMXPath $xpath, array $saved, string $doctorSignImage, string $patientSignImage, bool $forPdf = true): void
{
    ...
    foreach ($xpath->query('//img') as $img) {
        ...
        if ($doctorSignImage !== '' && $this->is_doctor_related_context($context)) {
            $normalized = $forPdf
                ? $this->normalize_signature_src_for_pdf($doctorSignImage)
                : $this->normalize_signature_src_for_view($doctorSignImage);
        ...
        if ($patientSignImage !== '' && $this->is_patient_related_context($context)) {
            $normalized = $forPdf
                ? $this->normalize_signature_src_for_pdf($patientSignImage)
                : $this->normalize_signature_src_for_view($patientSignImage);
            $img->setAttribute('src', $normalized);
        }
    }
}
...
// Same job as normalize_signature_src_for_pdf(), but for the
// interactive <img> shown in the browser (via #consentPdfFrame),
// where file:// paths are useless — the image must be a URL the
// browser can actually fetch over http(s).
private function normalize_signature_src_for_view(string $src): string
{
    $src = trim($src);
    if ($src === '') {
        return '';
    ...
    // base64 data URI — works directly in any <img src="">
    if (strpos($src, 'data:image/') === 0) {
        return $src;
    }

    // Already absolute http(s) — pass through as-is
    if (strpos($src, 'http://') === 0 || strpos($src, 'https://') === 0) {
        return $src;
    }

    // A file:// path (shouldn't normally reach here, but just in
    // case) — the browser can't load it, so there's nothing safe
    // to return.
    if (strpos($src, 'file://') === 0) {
        return '';
    }

    // Relative path starting with /uploads/ — resolve to a real
    // web URL under this site's base URL.
    if (strpos($src, '/uploads/') === 0) {
        return base_url(ltrim($src, '/'));
    }

    // Bare relative path (no leading slash) — same treatment.
    if (strpos($src, 'uploads/') === 0) {
        return base_url($src);
    }
...
private function collect_context_text_for_pdf(DOMXPath $xpath, DOMNode $node): string
{
    ...
    // textContent recurses into EVERY descendant. In templates where
    // a field sits directly under the page's one big wrapping <td>
    // (as a sibling of the profile table, section headings, and the
    // signature table — not nested inside its own small cell),
    // parentNode->textContent pulls in the ENTIRE page's text: every
    // label, every heading, everything. A field like an "incident
    // description" textarea could then inherit "patient" AND
    // "signature" from unrelated sections elsewhere on the page,
    // and get wrongly auto-filled with signature data. Only trust
    // parentNode text when the parent is a tight container — a <td>
    // holding just this field and maybe a caption, not an entire
    // page section.
    if ($node->parentNode && strtoupper($node->parentNode->nodeName ?? '') === 'TD') {
        $hasNestedSection = $xpath->query('.//table', $node->parentNode)->length > 0;
        if (!$hasNestedSection) {
            $chunks[] = strtolower(trim((string)$node->parentNode->textContent));
        }
    }

    // Rows like "<td>Patient Full Name</td><td><input></td><td>Case
    // Reference ID</td><td><input></td>" pack MULTIPLE label/value
    // pairs into one <tr>. Grabbing the whole row's text (as before)
    // meant EVERY field in that row inherited every OTHER field's
    // label too — e.g. "Case Reference ID" picking up "patient" from
    // "Patient Full Name" three cells over, and getting wrongly
    // auto-filled with the patient's name. Only the immediately
    // preceding sibling <td> is actually this field's label.
    $ownTd = $xpath->query('ancestor::td[1]', $node)->item(0);
    if ($ownTd) {
        $labelTd = $xpath->query('preceding-sibling::td[1]', $ownTd)->item(0);
        if ($labelTd) {
            $chunks[] = strtolower(trim((string)$labelTd->textContent));
        }
    }
...
{
    // "Local PCP / Doctor Name", "family doctor", "doctor back home",
    // etc. refer to the PATIENT'S OWN doctor, not the treating/
    // assigned MediTravel physician — never auto-fill those with
    // our resolved doctor_name even though the word "doctor" is
    // present. (Seen in MeditravelPostReturnClinicalFollowUpLog.html:
    // "Local PCP / Doctor Name".)
    if (preg_match('/\b(local|pcp|home|family)\b/i', $context)) {
        return false;
    }

    return (bool) preg_match('/\b(doctor|consultant|dr\.?|surgeon|surgen|physician)\b/i', $context);
}
...
private function is_patient_related_context(string $context): bool
{
    return (strpos($context, 'patient') !== false)
        ...
        // Some templates call the patient "the Client" instead
        // (e.g. MedicalTravelDisclaimer.html: "Signature of Client").
        || (strpos($context, 'client') !== false);
}
